package ltd.fdsa.starter.register.consul.autoconfig;

import com.ecwid.consul.v1.ConsulClient;
import com.ecwid.consul.v1.agent.model.NewService;

import lombok.extern.slf4j.Slf4j;
import ltd.fdsa.starter.register.annotation.AuthRole;
import ltd.fdsa.starter.register.consul.properties.ConsulAuthProperties;
import ltd.fdsa.starter.register.consul.properties.ConsulHealthCheckProperties;
import ltd.fdsa.starter.register.consul.properties.ConsulProperties;
...
//            }
	...
	private ConsulHealthCheckProperties getConsulHealthCheckProperties() {
		...
		try {
			consulHealthCheckProperties = Binder.get(environment)
					.bind(ConsulHealthCheckProperties.prefix, ConsulHealthCheckProperties.class).get();
		} catch (Exception e) {
	...
	private NewService createNewService() {
		...
		try {
			...
			if (consulHealthCheckProperties.getEnabled()) {
				...
				check.setHttp("http://" + applicationHost + ":" + applicationPort
						+ consulHealthCheckProperties.getHealthCheckPath());
				check.setMethod("GET");
				...
				check.setDeregisterCriticalServiceAfter(
						consulHealthCheckProperties.getDeRegisterCriticalServiceAfter());
				service.setCheck(check);
			}
		} catch (Exception e) {
			log.error("createNewService failed:{}", e.getLocalizedMessage());
		}
		return service;
	}
	...
	private ConsulAuthProperties getConsulAuthProperties() {
		...
		try {
			consulAuthProperties = Binder.get(environment).bind(ConsulProperties.prefix, ConsulAuthProperties.class)
					.get();
		} catch (Exception e) {
