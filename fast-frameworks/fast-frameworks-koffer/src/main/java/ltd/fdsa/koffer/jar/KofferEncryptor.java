 ...
 * 普通JAR包加密器
 */
public class KofferEncryptor extends XEntryEncryptor<JarArchiveEntry>
        implements XEncryptor, XConstants {
    private final int level;

    public KofferEncryptor(XEncryptor xEncryptor) {
        this(xEncryptor, new KofferAllEntryFilter());
    }

    public KofferEncryptor(XEncryptor xEncryptor, XEntryFilter<JarArchiveEntry> filter) {
        this(xEncryptor, Deflater.DEFAULT_COMPRESSION, filter);
    }

    public KofferEncryptor(XEncryptor xEncryptor, int level) {
        this(xEncryptor, level, new KofferAllEntryFilter());
    }

    public KofferEncryptor(XEncryptor xEncryptor, int level, XEntryFilter<JarArchiveEntry> filter) {
    ...
    public void encrypt(XKey key, InputStream in, OutputStream out) throws IOException {
        ...
        try {
            ...
            while ((entry = zis.getNextJarEntry()) != null) {
                ...
                if (entry.isDirectory()) {
                    ...
                    if (mainClass != null) {
                        ...
                        attributes.putValue("Main-Class", "cn.zhumingwu.koffer.jar.KofferLauncher");
                    }
                    JarArchiveEntry jarArchiveEntry = new JarArchiveEntry(entry.getName());
                    jarArchiveEntry.setTime(entry.getTime());
                    zos.putArchiveEntry(jarArchiveEntry);
            ...
            if (!indexes.isEmpty()) {
                JarArchiveEntry KofferInfDir = new JarArchiveEntry(Koffer_INF_DIR);
                KofferInfDir.setTime(System.currentTimeMillis());
                zos.putArchiveEntry(KofferInfDir);
                zos.closeArchiveEntry();

                JarArchiveEntry KofferInfIdx = new JarArchiveEntry(Koffer_INF_DIR + Koffer_INF_IDX);
