# koffer-Maven-Plugin [![](https://jitpack.io/v/core-lib/koffer-maven-plugin.svg)](https://jitpack.io/#core-lib/koffer-maven-plugin)
...
```
#### 也可以通过Maven命令单独执行 koffer 插件
```text
mvn koffer:build -Dkoffer.password=io.koffer
mvn koffer:build -Dkoffer.password=io.koffer -Dkoffer.targetDir=/directory/to/save/target.koffer
```

#### 但通常情况下是让koffer插件绑定到指定的phase中自动执行，这样就能在项目构建的时候自动构建出加密的包。
```text
mvn clean package -Dkoffer.password=io.koffer
mvn clean install -Dkoffer.password=io.koffer -Dkoffer.targetDir=/directory/to/save/target.koffer
```

## 强烈建议
强烈建议不要在 pom.xml 的 koffer-maven-plugin 配置中写上密码，这样会导致打包出来的 koffer 包中的 pom.xml 文件保留着密码，极其容易暴露密码！强烈推荐通过 mvn 命令来指定加密密钥！
...
| :------ | :----------- | :------ | :------ | :----- | :----- |
| password | -Dkoffer.password | 密码字符串 | String | 必须 | 任意字符串，io.koffer |
| algorithm | -Dkoffer.algorithm | 加密算法名称 | String | AES/CBC/PKCS5Padding | JDK内置加密算法，如：AES/CBC/PKCS5Padding 和 DES/CBC/PKCS5Padding |
| keySize | -Dkoffer.keySize | 密钥长度 | int | 128 | 根据加密算法而定，56，128，256 |
| ivSize | -Dkoffer.ivSize | 密钥向量长度 | int | 128 | 根据加密算法而定，128 |
| sourceDir | -Dkoffer.sourceDir | 源jar所在目录 | File | ${project.build.directory} | 文件目录 |
| sourceJar | -Dkoffer.sourceJar | 源jar名称 | String | ${project.build.finalName}.jar | 文件名称 |
| targetDir | -Dkoffer.targetDir | 目标jar存放目录 | File | ${project.build.directory} | 文件目录 |
| targetJar | -Dkoffer.targetJar | 目标jar名称 | String | ${project.build.finalName}.koffer | 文件名称 |
| includes | -Dkoffer.includes | 需要加密的资源路径表达式 | String[] | 无 | com/company/project/** , mapper/*Mapper.xml , 支持Ant表达式 |
| excludes | -Dkoffer.excludes | 无需加密的资源路径表达式 | String[] | 无 | static/** , META-INF/resources/** , 支持Ant表达式 |
| deletes | -Dkoffer.deletes | 加密后删除指定资源路径表达式 | String[] | 无 | target/\*.jar, ../module/target/\*.jar, 支持Ant表达式 |
