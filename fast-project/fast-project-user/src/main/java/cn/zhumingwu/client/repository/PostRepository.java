...
public class PostRepository {
    ...
    public int insert(Post... entities) {
        ...
        for (var entity : entities) {
            ...
            var categoryPath = entity.getCategoryPath();
            ...
            result += this.jdbcTemplate.update( sql, id,clientId,categoryPath,title,data,image,geometry,fromTime,endTime,createdTime,updatedTime,createdBy,updatedBy,status);
        }
        return result;
    }
    ...
    public int update(Post... entities) {
        ...
        String sql = "UPDATE t_post SET cid=?,\ncategory_path=?,\ntitle=?,\ndata=?,\nimage=?,\ngeometry=?,\nfrom_time=?,\nend_time=?,\ncreate_time=?,\nupdate_time=?,\ncreate_by=?,\nupdate_by=?,\nstatus=?\n WHERE post_id=? \n;";
        int result = 0;
        for (var entity : entities) {
            var id = entity.getId();
            var clientId = entity.getClientId();
            ...
            var updatedBy = entity.getUpdatedBy();
            var status = entity.getStatus();
            result += this.jdbcTemplate.update( sql, clientId,categoryPath,title,data,image,geometry,fromTime,endTime,createdTime,updatedTime,createdBy,updatedBy,status,id);
        }
        return result;
    }

    public int delete(int... ids) {
        String sql = "UPDATE t_post SET status = -1 WHERE post_id=? \n;";
        int result = 0;
        for (var id : ids) {
    ...
    List<Post> extractData(ResultSet rs)  {
        List<Post> list = new ArrayList<>();
		try {
            while (rs.next()) {
                Post entity = new Post();
                entity.setId(rs.getLong("post_id"));
                entity.setClientId(rs.getLong("cid"));
                entity.setCategoryPath(rs.getString("category_path"));
