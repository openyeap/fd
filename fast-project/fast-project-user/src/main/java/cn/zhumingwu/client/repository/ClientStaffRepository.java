...
@Service
@Slf4j
public class ClientStaffRepository {
    ...
    public List<ClientStaff> page(int index, int pageSize, String order, String where, Object... param) {
    ...
    public List<ClientStaff> query(String where, Object... param) {
        String sql = "SELECT staff_id,\ncid,\nuid,\nname,\ndescription,\nsort,\ndepartment_id,\nleader_user_id,\nphone,\nemail,\ntitle,\ncreate_time,\nupdate_time,\ncreate_by,\nupdate_by,\nstatus\nFROM t_client_staff " + where;
        return this.jdbcTemplate.query(sql, this::extractData, param);
    }

    public List<ClientStaff> queryByPrimaryKey(Object key) {
        String sql = "SELECT staff_id,\ncid,\nuid,\nname,\ndescription,\nsort,\ndepartment_id,\nleader_user_id,\nphone,\nemail,\ntitle,\ncreate_time,\nupdate_time,\ncreate_by,\nupdate_by,\nstatus\nFROM t_client_staff  WHERE staff_id=? and \ncid=? \n;";
        return this.jdbcTemplate.query(sql, this::extractData, key);
    }

    public int insert(ClientStaff... entities) {
        String sql = "INSERT INTO t_client_staff (staff_id,\ncid,\nuid,\nname,\ndescription,\nsort,\ndepartment_id,\nleader_user_id,\nphone,\nemail,\ntitle,\ncreate_time,\nupdate_time,\ncreate_by,\nupdate_by,\nstatus\n) VALUES (\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?\n)";
        int result = 0;
        for (var entity : entities) {
            ...
            var leaderId = entity.getLeaderId();
            ...
            result += this.jdbcTemplate.update( sql, id,cid,userId,name,description,sortNo,departmentId,leaderId,phone,email,title,createdTime,updatedTime,createdBy,updatedBy,status);
        }
        return result;
    }

    public int update(ClientStaff... entities) {

        String sql = "UPDATE t_client_staff SET uid=?,\nname=?,\ndescription=?,\nsort=?,\ndepartment_id=?,\nleader_user_id=?,\nphone=?,\nemail=?,\ntitle=?,\ncreate_time=?,\nupdate_time=?,\ncreate_by=?,\nupdate_by=?,\nstatus=?\n WHERE staff_id=? and \ncid=? \n;";
        int result = 0;
        for (var entity : entities) {
            var id = entity.getId();
            var userId = entity.getUserId();
            ...
            var updatedBy = entity.getUpdatedBy();
            var status = entity.getStatus();
            result += this.jdbcTemplate.update( sql, userId,name,description,sortNo,departmentId,leaderId,phone,email,title,createdTime,updatedTime,createdBy,updatedBy,status,id,cid);
        }
        return result;
    }

    public int delete(int... ids) {
        String sql = "UPDATE t_client_staff SET status = -1 WHERE staff_id=? and \ncid=? \n;";
        int result = 0;
        for (var id : ids) {
    ...
    public int deleteAll(Integer... ids) {
        String sql = "DELETE FROM t_client_staff WHERE staff_id=? and \ncid=? \n;";
    ...
    List<ClientStaff> extractData(ResultSet rs)  {
        List<ClientStaff> list = new ArrayList<>();
		try {
            while (rs.next()) {
                ClientStaff entity = new ClientStaff();
                entity.setId(rs.getLong("staff_id"));
                entity.setUserId(rs.getLong("uid"));
                entity.setName(rs.getString("name"));
                entity.setDescription(rs.getString("description"));
                entity.setSortNo(rs.getInt("sort"));
                entity.setDepartmentId(rs.getLong("department_id"));
                entity.setLeaderId(rs.getLong("leader_user_id"));
