...
## 目标
将数据流分流到集群中，从而充分利用集群的计算能力对后续数据进行处理。

## 流程

协调者将后续数据处理流程(Job Config)发送到集群中

集群得到Job Config创建后续处理流程管道，成功后返回协调者渠道编号（ChannelID)
协调者得到渠道编号后创建相关的调用客户端。
当协调者获得新数据时，根据用户配置的并行数轮询发给集群节点中。
to Cluster

var configurations = this.config.getConfigurations("pipelines");
if (configurations != null && configurations.length > 0) {
// invoke rpc to init
ApplicationContextHolder.registerBean(Pipeline.class,"test");
//删除bean.
