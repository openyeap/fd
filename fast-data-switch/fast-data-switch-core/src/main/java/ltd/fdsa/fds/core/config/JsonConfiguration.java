...
import ltd.fdsa.fds.util.StrUtil;
...
//			  	throw new FastDataSwitchException(
...
//		} catch (IOException e) {
//			throw new FastDataSwitchException(String.format("请检查您的配置文件. 您提供的配置文件读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", e));
//		}
//	}
	...
	public String getNecessaryValue(String key) {
		...
		if (StringUtils.isBlank(value)) {
			throw new FastDataSwitchException(String.format("您提供配置文件有误，[%s]是必填参数，不允许为空或者留白 .", key));
		}
	...
	public List<Configuration> getListConfiguration(final String path) {
		...
		for (final Object object : lists) {
			result.add(JsonConfiguration.from(JsonConfiguration.toJSONString(object)));
		}
		return result;
	}
	...
	public Map<String, Configuration> getMapConfiguration(final String path) {
		...
		for (final String key : map.keySet()) {
			result.put(key, JsonConfiguration.from(JsonConfiguration.toJSONString(map.get(key))));
		}
	...
	public Object remove(final String path) {
		...
		if (null == result) {
			throw new FastDataSwitchException(String.format("配置文件对应Key[%s]并不存在，该情况是代码编程错误. 请联系DataX团队的同学.", path));
		}
	...
	public String toJSON() {
		return JsonConfiguration.toJSONString(this.getInternal());
	}
	...
	public Configuration clone() {
		Configuration config = JsonConfiguration.from(JsonConfiguration.toJSONString(this.getInternal()));
	...
	private void setObject(final String path, final Object object) {
		...
		throw new FastDataSwitchException(String.format("值[%s]无法适配您提供[%s]， 该异常代表系统编程错误, 请联系DataX开发团队!",
	...
	Object buildObject(final List<String> paths, final Object object) {
		if (null == paths) {
			throw new FastDataSwitchException("Path不能为null，该异常代表系统编程错误, 请联系DataX开发团队 !");
		}
		...
		for (int i = paths.size() - 1; i >= 0; i--) {
			...
			throw new FastDataSwitchException(String.format("路径[%s]出现非法值类型[%s]，该异常代表系统编程错误, 请联系DataX开发团队! .",
	...
	Object setObjectRecursive(Object current, final List<String> paths, int index, final Object value) {
		...
		throw new FastDataSwitchException("该异常代表系统编程错误, 请联系DataX开发团队 !");
	}
	...
	private static void checkJSON(final String json) {
		if (StringUtils.isBlank(json)) {
			throw new FastDataSwitchException("配置信息错误. 因为您提供的配置信息不是合法的JSON格式, JSON不能为空白. 请按照标准json格式提供配置信息. ");
		}
	}
